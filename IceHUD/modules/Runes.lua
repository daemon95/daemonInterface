...
function Runes.prototype:GetOptions()
	...
	opts["runeArc"] = {
		type = 'range',
		name = "Rune arc",
		desc = "Bend rune row into arc (only works for graphical mode)",
		--name = L["Rune arc"],
		--desc = L["Bend rune row into arc (only works for graphical mode)"],
		min = -100,
		max = 100,
		step = 1,
		get = function()
			return self.moduleSettings.runeArc
		end,
		set = function(info, v)
			self.moduleSettings.runeArc = v
			self:Redraw()
		end,
		disabled = function()
			return not self.moduleSettings.enabled
...
function Runes.prototype:GetDefaultSettings()
	...
	defaults["displayMode"] = "Vertical"
	defaults["cooldownMode"] = "Alpha"
	defaults["runeGap"] = 0
	defaults["runeArc"] = 0
...
function Runes.prototype:CreateRune(i, type, name)
	...
	elseif i == 5 or i == 6 then
        ...
        local runeArc = self.moduleSettings.runeArc / 100
        local runeArcAbs = abs(runeArc)
        local runeIndex = 1 - (1 + 2 * (runeSwapI - 1)) / self.numRunes
        local runeAngle = runeIndex  * math.pi * runeArcAbs
        local runeStack = (self.numRunes * (self.runeSize - 5)) + self.numRunes + (self.numRunes * self.moduleSettings.runeGap)
        if not self.moduleSettings.runeArc then
            self.moduleSettings.runeArc = 0
        end
	if self.moduleSettings.displayMode == "Horizontal" then
            -- Horizontal runes alignment (with bend about circle)
		self.frame.graphical[i]:SetPoint("TOPLEFT", ((runeStack / 2) - (runeStack / (self.numRunes * 2))) - ((1 - runeArcAbs) * runeStack * runeIndex / 2) - (runeStack / (2 * math.pi) * math.sin(runeArcAbs * runeIndex * math.pi)), (runeArc / runeArcAbs) * (runeStack / (2 * math.pi) * (1 - math.cos(runeArcAbs * runeIndex * math.pi))))
	else
            -- Vertical runes alignment (with bend about circle)
		self.frame.graphical[i]:SetPoint("TOPLEFT", (runeArc / runeArcAbs) * (runeStack / (2 * math.pi) * (1 - math.cos(runeArcAbs * runeIndex * math.pi))), -((runeStack / 2) - (runeStack / (self.numRunes * 2))) + ((1 - runeArcAbs) * runeStack * runeIndex / 2) + (runeStack / (2 * math.pi) * math.sin(runeArcAbs * runeIndex * math.pi)))
	end
