...
function tt:VARIABLES_LOADED(event)
	...
	self:ApplySettings();
	-- Position
	if (cfg.anchorOffsetX and cfg.anchorOffsetY) then
		self:ClearAllPoints();
		self:SetPoint(cfg.anchorPoint or "CENTER", UIParent, cfg.anchorPoint or "CENTER", cfg.anchorOffsetX or 0, cfg.anchorOffsetY or 0);
	else
		self:Show();
		...
		_, _, _, cfg.anchorOffsetX, cfg.anchorOffsetY = self:GetPoint()
	end
	-- Cleanup
...
function tt:ApplySettings()
	-- Positioning Main Anchor Frame
	if (cfg.anchorPoint ~= select(1, self:GetPoint())) then
		self:ClearAllPoints()
		self:SetPoint(cfg.anchorPoint, UIParent, cfg.anchorPoint, function () cfg.anchorOffsetX = 0 return cfg.anchorOffsetX end, function () cfg.anchorOffsetY = 0 return cfg.anchorOffsetY end)
	end
