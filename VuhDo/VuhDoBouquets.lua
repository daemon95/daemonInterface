...
local tUnit;
local tEmptyInfo = { };
local function VUHDO_getEffectiveUnit(aUnit)
--	if (strfind(aUnit, "target", 1, true)) then
--		if (VUHDO_CUSTOM_INFO["fixResolveId"] == nil) then
--			return aUnit, VUHDO_CUSTOM_INFO;
--		else
...
local function VUHDO_evaluateBouquet(aUnit, aBouquetName)
	...
	tUnit, tInfo = VUHDO_getEffectiveUnit(aUnit);
...
local function VUHDO_unregisterAllBouquetListeners()
	table.wipe(VUHDO_REGISTERED_BOUQUETS);
end
...
function VUHDO_initAllEventBouquets()
	table.wipe(VUHDO_LAST_EVALUATED_BOUQUETS);
	for tUnit, _ in pairs(VUHDO_RAID) do
		VUHDO_updateBouquetsForEvent(tUnit, VUHDO_UPDATE_ALL);
	end

	VUHDO_updateBouquetsForEvent("focus", VUHDO_UPDATE_DC);
	VUHDO_updateBouquetsForEvent("target", VUHDO_UPDATE_DC);
end
...
function VUHDO_bouqetsChanged()
	table.wipe(VUHDO_EVENT_BOUQUETS);
	table.wipe(VUHDO_CYCLIC_BOUQUETS);
	table.wipe(VUHDO_LAST_EVALUATED_BOUQUETS);
...
--
local tName;
function VUHDO_isAnyBouquetInterstedIn(anUpdateMode)
	for tName, _ in pairs(VUHDO_REGISTERED_BOUQUETS) do
...
--
local function VUHDO_isHotInSlot(aSlotNum, aHotName)
	return VUHDO_PANEL_SETUP["HOTS"]["SLOTS"][aSlotNum] == aHotName;
end
...
local function VUHDO_isInAnyHotSlot(aHotName)
	for tSlotNum = 1, 9 do
		if (VUHDO_isHotInSlot(tSlotNum, aHotName)) then
			return true;
		end
	end
