...
function VUHDO_getModelType(aModelId)
	if ((aModelId >= VUHDO_ID_GROUP_1 and aModelId <= VUHDO_ID_GROUP_8) or aModelId == VUHDO_ID_GROUP_OWN) then
		return VUHDO_ID_TYPE_GROUP;
	elseif (aModelId >= VUHDO_ID_WARRIORS and aModelId <= VUHDO_ID_DEATH_KNIGHT) then
		return VUHDO_ID_TYPE_CLASS;
	elseif (aModelId == VUHDO_ID_UNDEFINED) then
	  return VUHDO_ID_TYPE_UNDEFINED;
	else
		return VUHDO_ID_TYPE_SPECIAL;
	end
end
...
function VUHDO_isInRange(aUnit)
	...
	elseif (VUHDO_CONFIG["RANGE_PESSIMISTIC"] or GetSpellInfo(VUHDO_CONFIG["RANGE_SPELL"]) == nil) then
