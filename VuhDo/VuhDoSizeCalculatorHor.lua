...
function resetSizeCalcCachesHor()
...
local function VUHDO_getAdditionalBottomHeight(aPanelNum)
    ...
    -- HoT icons
  	...
  	if (tHotCfg.show) then
			if  (tHotCfg.radioValue == 7 or tHotCfg.radioValue == 8) then
  			tBottomSpace = VUHDO_PANEL_SETUP[aPanelNum]["SCALING"].barHeight * VUHDO_PANEL_SETUP[aPanelNum]["HOTS"].size / 100;
  		end
  	end
...
function VUHDO_getHealButtonPosHor(aPlaceNum, aRowNo, aPanelNum)
	...
	if (tHots.show and tHots.radioValue == 1) then
		tHotslots = VUHDO_getNumHotSlots(aPanelNum);
		tButtonX = tButtonX + VUHDO_PANEL_SETUP[aPanelNum]["SCALING"].barHeight * VUHDO_PANEL_SETUP[aPanelNum]["HOTS"].size / 100 * tHotslots;
	end
