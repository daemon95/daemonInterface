  ...
  tNumIterations = 0;
	...
	while (true) do
	  ...
	  if (tInfo == nil) then
    	  ...
    	  if (tAnotherInfo == nil) then
    	    break;
				...
				if (VUHDO_isValidClusterUnit(tAnotherInfo)) then
      			...
      			if (tNumSamples > VUHDO_MAX_SAMPLES) then
      			  break;
      			end
					end
				end
				tNumIterations = tNumIterations + 1;
				if (tNumIterations > VUHDO_MAX_ITERATIONS) then
					break;
				end
			end -- for
		...
		if (tNumSamples > VUHDO_MAX_SAMPLES or tNumIterations > VUHDO_MAX_ITERATIONS) then
		  break;
		end
	end
