...
local function VUHDO_clearTooltipLines()
  VUHDO_TOOLTIP_AKT_LINE_LEFT = 1;
	...
	table.wipe(VUHDO_TEXT_SIZE_LEFT);
end
...
local function VUHDO_setTooltipLine(aText, anIsLeft, aLineNum, aColor, aTextSize)
	if (anIsLeft) then
		tText = VUHDO_GLOBAL["VuhDoTooltipTextL" .. aLineNum];
	else
		tText = VUHDO_GLOBAL["VuhDoTooltipTextR" .. aLineNum];
	end

	tText:SetText(aText);

	if (aColor ~= nil) then
		tText:SetTextColor(aColor["TR"], aColor["TG"], aColor["TB"], 1);
	end

	if (aTextSize ~= nil) then
		tText:SetFont(GameFontNormal:GetFont(), aTextSize);
		tText:SetShadowColor(0, 0, 0, 1);
		tText:SetShadowOffset(1, -0.5);
	end

	if (anIsLeft) then
		_, VUHDO_TEXT_SIZE_LEFT[aLineNum] = tText:GetFont();
	else
		tText:SetHeight(VUHDO_TEXT_SIZE_LEFT[aLineNum]);
	end
...
function VUHDO_updateTooltip()
	...
	tDistance = VUHDO_getDistanceBetween("player", VUHDO_TT_UNIT);
	if (tDistance ~= nil) then
		tDistance = floor(tDistance * 10) / 10 .. " " .. VUHDO_I18N_YARDS;
	elseif ("player" == VUHDO_TT_UNIT) then
		tDistance = "0.0 " .. VUHDO_I18N_YARDS;
	else
		tDistance = VUHDO_I18N_UNKNOWN;
	end
...
function VUHDO_showTooltip(aButton)
	...
	if (not tTipConfig["show"]
		...
		or (VUHDO_isInFight() and not tTipConfig["inFight"])) then
		return;
	end
...
function VuhDoTooltipOnMouseUp(aTooltip)
	...
	tPosition.x = VUHDO_roundCoords(tX);
	tPosition.y = VUHDO_roundCoords(tY);
