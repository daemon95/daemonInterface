...
local function VUHDO_positionTableHeaders(aPanel, aPanelNum)
	...
	if (VUHDO_isTableHeadersShowing(aPanelNum) or VUHDO_isTableFootersShowing(aPanelNum)) then
		tAnzCols = #tModel;

		if (tAnzCols > VUHDO_MAX_GROUPS_PER_PANEL) then
			tAnzCols = VUHDO_MAX_GROUPS_PER_PANEL;
		end
	else
		tAnzCols = 0;
	end
	...
	for tCnt  = 1, VUHDO_MAX_GROUPS_PER_PANEL do
		tHeader = VUHDO_getHeader(tCnt, aPanelNum);
		tHeader:SetWidth(tWidth * (sBarScaling.headerWidth / 100) + 0.01);
		tHeader:SetHeight(tHeight);

		tHealthBar = VUHDO_getHeaderBar(tHeader);
		--tHealthBar:SetMinMaxValues(0, 100);
		...
		tHeaderText:SetFont(tFont, tonumber(sHeaderColSetup.textSize), "OUTLINE");
		tHeader:Hide();
	end
	...
	for tCnt  = 1, tAnzCols do
		...
		tHeader:ClearAllPoints();
		tHeader:SetPoint("TOPLEFT", aPanel:GetName(), "TOPLEFT",  tX + tWidth  / 2 * (1 - (sBarScaling.headerWidth / 100)), -tY);
...
--
local tModelArray;
local tGroupArray, tModelIndex, tModelId;
local tMemberNum;
...
local function VUHDO_positionAllHealButtons(aPanel, aPanelNum)
	...
	for tModelIndex,  tModelId  in ipairs(tModelArray)  do
		...
		for _, tUnit  in ipairs(tGroupArray)  do
			...
			if (tButtonIndex  > VUHDO_MAX_BUTTONS_PANEL) then
				return;
			end
			...
			VUHDO_setupAllHealButtonAttributes(tHealButton, tUnit, false, VUHDO_ID_VEHICLES == tModelId, false);
...
local function VUHDO_initManaBar(aButton, aManaBar, aWidth, anIsForceBar)
	...
	tManaBouquet = VUHDO_INDICATOR_CONFIG["BOUQUETS"]["MANA_BAR"];
	if (anIsForceBar or tInfo == nil or tManaBouquet ~= "") then
...
local function VUHDO_initOverhealText(aHealthBar, aWidth)
	...
	tX = sOverhealTextSetup.xAdjust * aWidth / 100;
	tY = -sOverhealTextSetup.yAdjust * sBarScaling.barHeight / 100;
...
local function VUHDO_initHotBars()
	tHotBarHeight = sBarScaling.barHeight * sHotConfig["BARS"].width / 100;
...
local function VUHDO_initHotIcon(tCnt)
	VUHDO_getBarIconTimer(sHealthBar, tCnt):SetText("");
	VUHDO_getBarIconCounter(sHealthBar, tCnt):SetText("");
	tHotIcon  = VUHDO_getBarIcon(sHealthBar,  tCnt);
	...
	if (sHotConfig.radioValue >= 20) then
		tHotIcon:SetWidth(sHotIconSize  / 2);
		tHotIcon:SetHeight(sHotIconSize / 2);
	else
	...
	if (sHotConfig.show) then
		...
		elseif (sHotPos == 12) then
			tHotIcon:SetPoint("BOTTOMLEFT", sButton:GetName(), "BOTTOMLEFT",  tOffset, 0);  -- lb corner
		elseif (sHotPos == 11) then
			tHotIcon:SetPoint("BOTTOMRIGHT", sButton:GetName(), "BOTTOMRIGHT", -tOffset, 0);  -- rb corner
		...
		elseif (sHotPos == 20) then
			...
			elseif (tCnt  == 2) then
				if (tIsBothTop)  then
					tHotIcon:SetPoint("TOP",  sHealthBar:GetName(), "TOP",  -sBarScaling.barWidth / 5, 0);
				else
			...
			elseif (tCnt  == 4) then
				if (tIsBothBottom)  then
					tHotIcon:SetPoint("BOTTOM", sHealthBar:GetName(), "BOTTOM", sBarScaling.barWidth / 5, 0);
				else
	...
	tTimer  = VUHDO_getBarIconTimer(sHealthBar, tCnt);
	tCounter  = VUHDO_getBarIconCounter(sHealthBar, tCnt);
	...
	if (tHotName  ~= nil) then
		...
		tChargeIcon = VUHDO_getBarIconCharge(sHealthBar, tCnt);
...
local function VUHDO_initCustomDebuffs()
	...
	tX = tDebuffConfig.xAdjust * sBarScaling.barWidth / 100;
	tY = -tDebuffConfig.yAdjust * sBarScaling.barHeight / 100;
	...
	for tCnt =  0, 4  do
		tIcon = VUHDO_getBarIcon(sHealthBar, 40 + tCnt);
		...
		tTimer = VUHDO_getBarIconTimer(sHealthBar, tCnt + 40);
		tTimer:ClearAllPoints();
		...
		tCounter = VUHDO_getBarIconCounter(sHealthBar, tCnt + 40);
		tCounter:ClearAllPoints();
		...
		tName = VUHDO_getBarIconName(sHealthBar, tCnt + 40);
...
local function VUHDO_initRaidIcon(aHealthBar, anIcon, aWidth)
	tX = sRaidIconSetup.xAdjust * aWidth / 100;
	tY = -sRaidIconSetup.yAdjust * sBarScaling.barHeight / 100;
...
local function VUHDO_initSwiftmendIndicator()
	...
	tIcon:SetWidth(sHotIconSize / 2 * VUHDO_INDICATOR_CONFIG["CUSTOM"]["SWIFTMEND_INDICATOR"]["SCALE"]);
	tIcon:SetHeight(sHotIconSize / 2 * VUHDO_INDICATOR_CONFIG["CUSTOM"]["SWIFTMEND_INDICATOR"]["SCALE"]);
	tIcon:Hide();
end
...
local function VUHDO_initHighlightBar()
	VUHDO_getHealthBar(sButton, 8):SetAlpha(0);
	VUHDO_getHealthBar(sButton, 8):Show();
	VUHDO_setLlcStatusBarTexture(VUHDO_getHealthBar(sButton, 8), VUHDO_INDICATOR_CONFIG["CUSTOM"]["MOUSEOVER_HIGHLIGHT"]["TEXTURE"]);
	VUHDO_getHealthBar(sButton, 8):SetValue(0);
end
...
function VUHDO_initHealButton(aButton, aPanelNum)
	...
	for tCnt =  1, 16 do
		...
		if (VUHDO_PANEL_SETUP["SCALING"].vertical and tCnt ~= 2  and tCnt ~= 7 and (tCnt < 9 or tCnt == 14)) then
			if (VUHDO_PANEL_SETUP["SCALING"].turnAxis) then
				tHealthBar:SetOrientation("VERTICAL_INV");
			else
				tHealthBar:SetOrientation("VERTICAL");
			end
		else
			if (VUHDO_PANEL_SETUP["SCALING"].turnAxis) then
				tHealthBar:SetOrientation("HORIZONTAL_INV");
			else
				tHealthBar:SetOrientation("HORIZONTAL");
			end
		end

		tHealthBar:SetIsInverted(VUHDO_PANEL_SETUP["SCALING"].invertGrowth and VUHDO_INVERT_BARS[tCnt]);
	end
...
local function VUHDO_initAllHealButtons(aPanel, aPanelNum)
	...
	for tCnt  = tNumButtons + 1, VUHDO_MAX_BUTTONS_PANEL do
...
function VUHDO_reloadUI()
	...
	if (not VUHDO_IN_COMBAT_RELOG) then
		table.wipe(VUHDO_FAST_ACCESS_ACTIONS);
	end
