...
local function VUHDO_customizeHotIcons(aButton, aHotName, aRest, aTimes, anIcon, aDuration, aMode, aShieldCharges, aColor, anIndex)
	...
	tPrefix = VUHDO_getHealthBar(aButton, 1):GetName() .. "Ic" .. anIndex;
	tIcon = VUHDO_GLOBAL[tPrefix .. "I"];
	tTimer = VUHDO_GLOBAL[tPrefix .. "T"];
	tCounter = VUHDO_GLOBAL[tPrefix .. "C"];
	tChargeTexture = VUHDO_GLOBAL[tPrefix .. "A"];
	...
	for tIndex, tHotName in pairs(sHotSlots) do
		if (aHotName == tHotName) then
			...
			if (aMode == 0 or aColor ~= nil) then -- Bouquet => color ~= nil
				...
				tIsMatch =
						(aMode == 1 and tSlotCfg["mine"] and not tSlotCfg["others"])
...
function VUHDO_removeHots(aUnit)
	...
	for _, tButton in pairs(tAllButtons) do
		tHbPrefix = VUHDO_getHealthBar(tButton, 1):GetName() .. "Ic";
		for tCnt = 1, 5 do
			tPrefix = tHbPrefix .. tCnt;
			VUHDO_GLOBAL[tPrefix  .. "I"]:SetAlpha(0);
			VUHDO_GLOBAL[tPrefix  .. "T"]:SetText("");
			VUHDO_GLOBAL[tPrefix  .. "A"]:Hide();
...
local function VUHDO_snapshotHot(aHotName, aRest, aStacks, anIcon, anIsMine, aDuration, aUnit)
	if (not VUHDO_ACTIVE_HOTS[aHotName]) then
		return;
	end
...
function VUHDO_updateAllHoTs()
	...
	table.wipe(VUHDO_SWIFTMEND_UNITS);
