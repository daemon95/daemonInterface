...
local VUHDO_getHealButtonWidth;
...
function VUHDO_panelRefreshInitBurst()
  ...
  VUHDO_getHealButtonWidth = VUHDO_GLOBAL["VUHDO_getHealButtonWidth"];
...
local function VUHDO_refreshPositionAllHealButtons(aPanel, aPanelNum)
  ...
  for tModelIndex, tModelId in ipairs(tModels) do
      ...
      if (tButton["raidid"] ~= tUnit) then
				VUHDO_setupAllHealButtonAttributes(tButton, tUnit, false, VUHDO_ID_VEHICLES == tModelId, false);
...
function VUHDO_refreshUI()
	...
	if (VUHDO_INTERNAL_TOGGLES[VUHDO_UPDATE_UNIT_TARGET]) then
