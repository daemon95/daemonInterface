	...
	if (tAmountInc > 0) then
  	...
  	for _, tButton in pairs(tAllButtons) do
    	tIncBar = VUHDO_getHealthBar(tButton, 6);
			...
			if (VUHDO_PANEL_SETUP["SCALING"]["invertGrowth"] and tInfo["healthmax"] > 0) then
		...
		for _, tButton in pairs(tAllButtons) do
			if (VUHDO_PANEL_SETUP["SCALING"]["invertGrowth"]) then
...
function VUHDO_customizeText(aButton, aMode, anIsTarget)
  ...
  tLifeConfig = tSetup["LIFE_TEXT"];
	...
	if (tIsLife and tIsShowLife) then
		...
		if (VUHDO_CONFIG["SHOW_TEXT_OVERHEAL"]) then
  ...
  -- Aggro Text
  if (tIsName) then
	  if (tInfo["aggro"] and VUHDO_THREAT_CFG["AGGRO_USE_TEXT"]) then
			tTextString =
...
function VUHDO_customizeHealButton(aButton)
	VUHDO_customizeText(aButton, VUHDO_UPDATE_ALL, false);

  tSetup = VUHDO_PANEL_SETUP[VUHDO_BUTTON_CACHE[aButton]];
	tUnit, _ = VUHDO_getDisplayUnit(aButton);
	-- Raid icon
	if (tSetup["RAID_ICON"]["show"] and tUnit ~= nil) then
...
--
local tInfo, tPrefix, tCnt, tAlpha;
local function VUHDO_customizeDebuffIconsRange(aButton)
  ...
  if (tInfo ~= nil) then
    if (tInfo["range"]) then
  	...
  	tPrefix = VUHDO_getHealthBar(aButton, 1):GetName() .. "Ic4";
  	for tCnt = 0, 4 do
  		VUHDO_GLOBAL[tPrefix .. tCnt]:SetAlpha(tAlpha);
  	end
  end
end
...
function VUHDO_updateHealthBarsFor(aUnit, anUpdateMode)
  ...
  tAllButtons = VUHDO_getUnitButtons(aUnit);
	...
	elseif (7 == anUpdateMode) then -- VUHDO_UPDATE_AGGRO
		if (VUHDO_CONFIG["THREAT"]["AGGRO_USE_TEXT"]) then
			for _, tButton in pairs(tAllButtons) do
	...
	elseif (10 == anUpdateMode) then -- VUHDO_UPDATE_ALIVE
		...
		VUHDO_updateIncHeal(aUnit);
	elseif (11 == anUpdateMode) then -- VUHDO_UPDATE_EMERGENCY
  ...
  tAllButtons = VUHDO_IN_RAID_TARGET_BUTTONS[tInfo["name"]];
	...
	elseif (10 == anUpdateMode) then -- VUHDO_UPDATE_ALIVE
		for _, tButton in pairs(tAllButtons) do
			VUHDO_customizeText(tButton, VUHDO_UPDATE_ALL, true);
		end
	elseif (1 == anUpdateMode) then -- VUHDO_UPDATE_ALL
		for _, tButton in pairs(tAllButtons) do
			VUHDO_customizeText(tButton, VUHDO_UPDATE_ALL, true);
