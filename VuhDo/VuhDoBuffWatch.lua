...
function VUHDO_buffWatchOnMouseUp(aPanel)
	...
	if (VUHDO_mayMoveHealPanels()) then
		...
		tOrientation, _, tRelative, tX, tY = aPanel:GetPoint(0);

		tCoords.x = VUHDO_roundCoords(tX);
		tCoords.y = VUHDO_roundCoords(tY);
		tCoords.relativePoint = tRelative;
		tCoords.point = tOrientation;
	end
end
...
function VUHDO_buffSelectDropdown_Initialize(_, _)
	if (VUHDO_CLICKED_BUFF == nil or VUHDO_CLICKED_TARGET == nil or VUHDO_isInFight()) then
		return;
	end
...
function VuhDoBuffPreClick(aButton, aMouseButton)
	...
	if (VUHDO_IS_USED_SMART_BUFF) then
		...
		VUHDO_setupAllBuffButtonsTo(aButton, tBuff, tTarget, VUHDO_getBuffVariantMaxTarget(tVariants)[1]);
		--VUHDO_setupAllBuffButtonUnits(aButton, tTarget);
	end
...
function VUHDO_getAllUniqueSpells()
	table.wipe(tUniqueBuffs);
	table.wipe(tUniqueCategs);
...
--	else
	if ("C" == tCode) then
		tGroup = VUHDO_BUFF_GROUPS[strsub(aTargetCode, 2)];
		return #tGroup == 0;
	else
		return false;
	end
...
local function VUHDO_getMissingBuffs(someBuffVariants, someUnits, aCategSpec)
	...
	table.wipe(tMissGroup);
	table.wipe(tLowGroup);
	table.wipe(tOkayGroup);
	table.wipe(tOorGroup);
	tGoodTarget = nil;
	tLowestRest = nil;
	tLowestUnit = nil;
	tNow = GetTime();
...
--			tInfo = VUHDO_RAID[VUHDO_PLAYER_RAID_ID];
--		else
...
--	else
	...
	elseif ("N" == tCode) then
		...
		elseif (VUHDO_BUFF_TARGET_ENCHANT == tMaxTarget) then
			...
			tHasEnch1, tEnchDuration1, _, tHasEnch2, tEnchDuration2, _ = GetWeaponEnchantInfo();

			if (tHasEnch1) then
				return {}, {}, "player", tEnchDuration1 * 0.001, "player", { "player" }, { }, 0;
			elseif (tHasEnch2) then
...
local function VUHDO_setBuffSwatchTimer(aSwatchName, aSecsNum, aCount)
	...
	if (aSecsNum ~= nil and aSecsNum >= 0) then
		...
		if (aCount ~= nil and aCount > 0 and not VUHDO_BUFF_SETTINGS["CONFIG"]["HIDE_CHARGES"]) then
...
function VUHDO_updateBuffSwatch(aSwatch)
	...
	if (VUHDO_isBuffGroupEmpty(tTargetCode)) then
		...
		if (tCooldown > 1.5) then
			...
			if (VUHDO_BUFFS[tRefSpell].wasOnCd
	...
	VUHDO_NUM_LOWS[aSwatch:GetName()] = #tLowGroup + #tMissGroup;
end
...
function VUHDO_updateBuffPanel()
	...
	table.wipe(VUHDO_OLD_MISS_BUFFS);
...
function VUHDO_execSmartBuffPre(self)
	...
	if (tCooldown > 0) then
		--UIErrorsFrame:AddMessage(VUHDO_I18N_SMARTBUFF_ERR_5, 1, 0.1, 0.1, 1);
		return;
	end

--	if (tMaxLowTarget == "player") then
--		tMaxLowTarget = VUHDO_PLAYER_RAID_ID;
--	end
