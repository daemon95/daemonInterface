...
local function VUHDO_customizeTargetBar(aButton, anIsFriend, anIsEnemy, anIcon, anIsDead, anIconIndex)
	...
	elseif (VUHDO_PANEL_SETUP["BAR_COLORS"]["RAID_ICONS"]["enable"] and anIconIndex ~= nil and anIcon ~= nil) then
	...
	VUHDO_customizeText(aButton, VUHDO_UPDATE_ALL, true);
...
function VUHDO_updateTargetBars(aUnit)
  ...
  tAllButtons = VUHDO_getUnitButtons(aUnit);
	...
	for _, tButton in pairs(tAllButtons) do
		...
		VUHDO_forgetTargetButton(tTarget, tTargetButton);

		tTotButton = VUHDO_getTotButton(tButton);
		VUHDO_forgetTargetButton(tTarget .. "target", tTotButton);
	end

	VUHDO_IN_RAID_TARGETS[tTarget] = nil;
	VUHDO_IN_RAID_TARGETS[tTarget .. "target"] = nil;
	...
	for _, tButton in pairs(tAllButtons) do
		...
		if (tConfig.showTot) then
  		...
  		VUHDO_customizeTargetBar(tTotButton, tIsFriend, tIsEnemy, tDispIcon, tIsDead, tostring(tDispIcon));
			...
			VUHDO_rememberTargetButton(tTarget, tTotButton);
		end
	end
end
...
function VUHDO_rebuildTargets()
	if (not VUHDO_INTERNAL_TOGGLES[VUHDO_UPDATE_UNIT_TARGET]) then
		return;
	end

	table.wipe(VUHDO_IN_RAID_TARGETS);
	table.wipe(VUHDO_IN_RAID_TARGET_BUTTONS);
	table.wipe(VUHDO_TOT_GUIDS);
