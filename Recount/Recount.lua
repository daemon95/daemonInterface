...
local Default_Profile={
	profile={
		Colors={
			["Window"]={
				["Title"] = { r = 0.975, g = 0.98, b = 1, a = 0.2},
				["Background"]= { r = 0.095, g = 0.095, b = 0.095, a = 0.2},
...
--				["GUARDIAN"] = { r = 0.61, g = 0.09, b = 0.09 },
		...
		MainWindow={
			...
			ShowScrollbar=false, -- Elsia: Allow toggle of scrollbar
			HideTotalBar=true,
			BarText=
			{
				RankNum =true,
...
-- Textures registration - TODO: Override
...
Recount.consoleOptions = {
	...
	args = {
		...
		FrameStrata = {
			...
			desc = L["Controls the frame strata of the Recount windows. Default: BACKGROUND"],
			...
			get = function(info) return Recount.db.profile.FrameStrata or "1-BACKGROUND" end,
...
--	Recount.db2 = acedb:New("RecountPerCharDB", DefaultConfig)
	...
	-- Setting up GraphWindow on initialization
	if Recount.db.profile.GraphWindowX then
		Recount.GraphWindow:ClearAllPoints()
		Recount.GraphWindow:SetPoint("TOPLEFT",UIParent,"TOPLEFT",Recount.db.profile.GraphWindowX,Recount.db.profile.GraphWindowY)
	end

	-- Setting up DetailWindow on initialization
	if Recount.db.profile.DetailWindowX then
		Recount.DetailWindow:ClearAllPoints()
		Recount.DetailWindow:SetPoint("TOPLEFT",UIParent,"TOPLEFT",Recount.db.profile.DetailWindowX,Recount.db.profile.DetailWindowY)
	end

	-- Setting up RealtimeWindows on initialization
	if Recount.db.profile.RealtimeWindows then
		local Windows=Recount.db.profile.RealtimeWindows
		for k,v in pairs(Windows) do
			if v[8] and v[8] == true then -- Elsia: Make sure to respect closed windows as closed on startup
...
--	Recount.LatestGuardian=0
	...
	-- Configuring Windows on Start-Up
...
function Recount:ResetPositions()
	...
	-- Add MainWindow positioning
	Recount.MainWindow:ClearAllPoints()
	--Recount.MainWindow:SetPoint("CENTER",UIParent,"CENTER",346,-296)
	Recount.MainWindow:SetPoint("BOTTOMRIGHT",UIParent,"BOTTOMRIGHT",-38,42)
	-- Reposition the rest windows to the center of the screen
	Recount.DetailWindow:ClearAllPoints()
	-- SetPoint("Child Point", Parent, "Parent Point", X, Y)
	Recount.DetailWindow:SetPoint("TOPLEFT",UIParent,"TOPLEFT",0,-16)
	Recount.GraphWindow:ClearAllPoints()
