...
local WIDTH_TAB = 660
local WIDTH_SECTION = WIDTH_TAB / 3
...
local ZoneLabels = { -- (1.2.x) Filters
...
local GroupLabels = { -- (1.2.x) Filters
	[1] = L["Solo"],
	[2] = L["Party"],
	[3] = L["Raid"],
}

local ZoneOrder = { -- For ZoneLabels
	"none", "party", "raid", "pvp", "arena"
}

-- SECTION: Recount functions
...
function Recount:ResetDefaultWindowColors()
	Recount.Colors:SetColor("Window", "Title", { r = 0.975, g = 0.98, b = 1, a = 0.2})
	Recount.Colors:SetColor("Window", "Background", { r = 0.095, g = 0.095, b = 0.095, a = 0.2})
	Recount.Colors:SetColor("Window", "Title Text", { r = 1, g = 1, b = 1, a = 1})
	Recount.Colors:SetColor("Other Windows", "Title", { r = 0.975, g = 0.98, b = 1, a = 0.2})
	Recount.Colors:SetColor("Other Windows", "Background", { r = 0.095, g = 0.095, b = 0.095, a = 0.2})
...
function me:HideOptions()
	...
	me:HighLightTab(me.ConfigWindow.Data.Tab.Background, false)
	me.ConfigWindow.Appearance:Hide()
	--me.ConfigWindow.Appearance.Tab.Background:SetVertexColor(1.0,0.2,0.2)
	me:HighLightTab(me.ConfigWindow.Appearance.Tab.Background, false)
	me.ConfigWindow.Window:Hide()
	--me.ConfigWindow.Window.Tab.Background:SetVertexColor(1.0,0.2,0.2)
	me:HighLightTab(me.ConfigWindow.Window.Tab.Background, false)
	me.ConfigWindow.ColorOpt:Hide()
	--me.ConfigWindow.ColorOpt.Tab.Background:SetVertexColor(1.0,0.2,0.2)
	me:HighLightTab(me.ConfigWindow.ColorOpt.Tab.Background, false)
	me.ConfigWindow.Profiles:Hide()
	--me.ConfigWindow.Profiles.Tab.Background:SetVertexColor(1.0,0.2,0.2)
	me:HighLightTab(me.ConfigWindow.Profiles.Tab.Background, false)
end

function me:HighLightTab(tabTexture, highlight) -- Setting Color of the given Tab (Highlight/Normal)
	if highlight == true then
		tabTexture:SetVertexColor(0.66, 0.66, 0.66)
	else
		tabTexture:SetVertexColor(0.33, 0.33, 0.33)
	end
end
...
--	me.WindowOptions.ShowCurAndLast:SetChecked(Recount.db.profile.Window.ShowCurAndLast)
	...
	me.WindowOptions.ClampToScr:SetChecked(Recount.db.profile.ClampToScreen)
...
function me:CreateConfigWindow() -- (*) GUI_Config Window
	--me.ConfigWindow=Recount:CreateFrame("Recount_ConfigWindow",L["Config Recount"],286+16,600)
	me.ConfigWindow = Recount:CreateFrame("Recount_ConfigWindow", L["Config Recount"], 286 + 16, WIDTH_TAB)
	...
	Recount.Colors:RegisterTexture("Other Windows", "Title", Graph:DrawLine(theFrame, WIDTH_SECTION, 12, WIDTH_SECTION, 233 + 16, 24, {0.5, 0.0, 0.0, 1.0}, "ARTWORK"), {r = 0.5, g = 0.5, b = 0.5, a = 1}) -- Elsia: Changed 32->12 for longer separators given no save/revert
	Recount.Colors:RegisterTexture("Other Windows", "Title", Graph:DrawLine(theFrame, WIDTH_SECTION * 2, 12, WIDTH_SECTION * 2, 233 + 16, 24, {0.5, 0.0, 0.0, 1.0}, "ARTWORK"), {r = 0.5, g = 0.5, b = 0.5, a = 1})
	Recount.Colors:RegisterTexture("Other Windows", "Title", Graph:DrawLine(theFrame, 2, 233 + 16, WIDTH_TAB - 2, 233 + 16, 24, {0.5, 0.0, 0.0, 1.0}, "ARTWORK"), {r = 0.5, g = 0.5, b = 0.5, a = 1})
	...
	me:CreateProfilesOptions(theFrame)
...
function me:CreateDataOptions(parent) -- (1) DataConfig Tab
	local theFrame = CreateFrame("FRAME", nil, parent)
	parent.Data = theFrame

	--theFrame:SetWidth(600)
	theFrame:SetHeight(200) -- 220
	theFrame:SetWidth(WIDTH_TAB)
	...
	Tab:SetScript("OnMouseDown", function(this) me:HideOptions(); theFrame:Show(); me:HighLightTab(this.Background, true) end)
	...
	me:HighLightTab(Tab.Background, false)
...
function me:CreateWindowOptions(parent) -- (2) WindowConfig Tab
	local theFrame = CreateFrame("FRAME", nil, parent)
...
function me:CreateAppearanceOptions(parent) -- (3) AppearanceConfig Tab
	local theFrame = CreateFrame("FRAME", nil, parent)
...
function me:CreateColorOptions(parent) -- (4) ColorConfig Tab
	local theFrame = CreateFrame("FRAME", nil, parent)
...
function me:CreateProfilesOptions(parent) -- (5) ProfilesConfig Tab
	local theFrame = CreateFrame("FRAME", nil, parent)
	parent.Profiles = theFrame
	...
	-- Profiles Config Tab
	local Tab = CreateFrame("FRAME", nil, parent)
	parent.Profiles.Tab = Tab

	Tab:SetWidth(100)
	Tab:SetHeight(18)
	Tab:SetPoint("TOPLEFT", parent, "TOPLEFT", 412, -35)
	Tab:EnableMouse(true)
	--Tab:SetScript("OnMouseDown", function(this) me:HideOptions(); theFrame:Show(); this.Background:SetVertexColor(0.2, 1.0, 0.2) end)
	Tab:SetScript("OnMouseDown", function(this) me:HideOptions(); theFrame:Show(); me:HighLightTab(this.Background, true) end)
	Tab.Text=Tab:CreateFontString(nil, "OVERLAY", "GameFontNormal")
...
function me:SetupFilterOptions(parent) -- (1.1) Filter Config-Column
	...
	-- Filter Header Icons Positioning (from Right to Left / reverse Order)
	...
	-- Filter Header Positioning within the Column
...
function me:SetupMiscOptions(parent) -- (1.2) Version-Filter Config-Column
	me.MiscOptions=CreateFrame("Frame", nil, parent)
	local theFrame=me.MiscOptions

	-- Config-Column Positioning
	theFrame:SetHeight(200)
	--theFrame:SetWidth(200)
	theFrame:SetWidth(WIDTH_SECTION)
	theFrame:SetPoint("TOPLEFT", parent, "TOPLEFT", WIDTH_SECTION, -34) -- The Second Config-Column

	-- Version Block
	theFrame.Title=theFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	--theFrame.Title:SetText(L["Recount Version"])
	theFrame.Title:SetText("Recount |cffeeeeee3.3.5a|r |cbf7f6699Eidolon|r |c33333333Mod|r")
	--theFrame.Title:SetPoint("TOP", theFrame, "TOP", -20, -2)
	--theFrame.Title:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 16, -2)
	...
	--theFrame.VersionText:SetText("3.3.5a |cbf7f6699Eidolon Mod|r")
	--theFrame.VersionText:SetPoint("LEFT", theFrame.Title, "RIGHT" , 4, 0) -- TOP theFrame TOP -20
	-- Extra Label (after Version)
	--theFrame.ExtraText=theFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	--theFrame.ExtraText:SetTextColor(0.5, 0.4, 0.6, 0.75)
	--theFrame.ExtraText:SetText("Eidolon")
	--theFrame.ExtraText:SetPoint("LEFT", theFrame.VersionText, "RIGHT" , 4, 0)
	
	-- Version Check "VerChk" Button
	...
	-- Filters Block
	local upperGap, rowHeight = -37, 17
	...
	theFrame.Title2:SetPoint("TOP", theFrame, "TOP", 0, upperGap + rowHeight) -- + 14
	-- Zone-Based Filters Checkboxes
	local i = 0
	for _, k in pairs(ZoneOrder) do
		...
		theFrame[k]:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 10, upperGap - i * rowHeight)
	...
	-- Group-Based Filters Checkboxes
	for k, v in ipairs(GroupLabels) do
...
function me:SetupDeletionOptions(parent) -- (1.3) DataDeletion Config-Column
	me.DeletionOptions=CreateFrame("Frame",nil,parent)
	local theFrame=me.DeletionOptions
	
	-- Positioning Section within the Tab
	theFrame:SetHeight(200)
	--theFrame:SetWidth(200)
	theFrame:SetWidth(WIDTH_SECTION)
	theFrame:SetPoint("TOPLEFT", parent, "TOPLEFT", WIDTH_SECTION * 2, -34) -- The Third Config-Column
...
function me:SetupButtonOptions(parent) -- (2.1) MainWindow Config-Column
	...
	-- CloseButton SECTION END
	
	-- TODO Insert Extra Ace3Config Dialog button
...
function me:SetupWindowOptions(parent) -- (2.2) Position-Scale Config-Column
	...
	theFrame.ResetWinButton:SetWidth((WIDTH_SECTION - 16) / 2)
	theFrame.ResetWinButton:SetHeight(24)
	--theFrame.ResetWinButton:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 40, -20)
	theFrame.ResetWinButton:SetPoint("TOPLEFT", theFrame, "TOP", 4, -20)
	...
	theFrame.ToggleButton = CreateFrame("Button", nil, theFrame, "OptionsButtonTemplate")
	theFrame.ToggleButton:SetWidth((WIDTH_SECTION - 16) / 2)
	theFrame.ToggleButton:SetHeight(24)
	theFrame.ToggleButton:SetPoint("TOPRIGHT", theFrame, "TOP", -4, -20)
	theFrame.ToggleButton:SetScript("OnClick", function() if Recount.MainWindow:IsShown() then Recount.MainWindow:Hide() else Recount.MainWindow:Show(); Recount:RefreshMainWindow() end end)
	theFrame.ToggleButton:SetText(L["Toggle"])
	-- Window Scale Slider
	...
	slider:SetWidth(180)
	slider:SetHeight(16)
	slider:SetPoint("TOP", theFrame, "TOP", 0, -60)
...
--[[	theFrame.ShowCurAndLast=me:CreateSavedCheckbox(L["Autoswitch Shown Fight"],theFrame,"Window","ShowCurAndLast")
	...
	theFrame.LockWin:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 8, -88)
	...
	-- Snap to Screen Checkbox
	theFrame.ClampToScr = me:CreateSavedCheckbox(L["Clamp To Screen"], theFrame, "Window", "ClampToScr")
	theFrame.ClampToScr:SetPoint("TOPLEFT", theFrame.LockWin, "TOPLEFT", 0, -20)
	theFrame.ClampToScr:SetScript("OnClick", function(this) if this:GetChecked() then this:SetChecked(true); Recount.db.profile.ClampToScreen = true; Recount:SetStrataAndClamp(); else this:SetChecked(false); Recount.db.profile.ClampToScreen = false; Recount:SetStrataAndClamp(); end end)

	--local dropdown = CreateFrame("Dropdown", "Recount_ConfigWindow_Strata_Dropdown", theFrame, "OptionsEditBoxTemplate")
	--theFrame.StrataDropdown = dropdown
	--dropdown:SetOrientation("HORIZONTAL")
	--dropdown:SetMinMaxValues(0.5, 1.5)
	--dropdown:SetValueStep(0.05)
	--dropdown:SetWidth(180)
	--dropdown:SetHeight(16)
	--dropdown:SetPoint("TOP", theFrame, "TOP", 0, -100)
	--dropdown:SetScript("OnValueChanged", function(this) Recount.db.profile.Scaling = math.floor(this:GetValue() * 100 + 0.5) / 100; getglobal(this:GetName() .. "Text"):SetText(L["Window Scaling"] .. ": " .. Recount.db.profile.Scaling); Recount:ScaleWindows(Recount.db.profile.Scaling) end)
	--dropdown:SetScript("OnMouseUp", function() me:ScaleConfigWindow(Recount.db.profile.Scaling) end)
	--getglobal(slider:GetName() .. "High"):SetText("1.5");
	--getglobal(slider:GetName() .. "Low"):SetText("0.5");
	--getglobal(slider:GetName() .. "Text"):SetText(L["Window Scaling"] .. ": " .. Recount.db.profile.Scaling)
...
function me:SetupRealtimeOptions(parent) -- (2.3) Realtime Config-Column
	me.RealtimeOptions=CreateFrame("Frame",nil,parent)
	local theFrame=me.RealtimeOptions
	theFrame:SetHeight(200)
	--theFrame:SetWidth(200)
	theFrame:SetWidth(220)
...
function me:CreateBarSelection(parent) -- (3.1) Bars Config-Column
...
function me:CreateFontSelection(parent) -- (3.2) Font Config-Column
	...
	if table.getn(Fonts) <= 13 then
		for i = 1, 13 do
			theFrame.Rows[i]:SetWidth(WIDTH_SECTION - 4)
...
function me:CreateTextureSelection(parent) -- (3.3) BarTexture Section
...
function me:CreateWindowColorSelection(parent) -- (4.1) WindowColor Config-Column
	...
	theFrame.ResetColButton:SetWidth(WIDTH_SECTION / 2)
	--theFrame.ResetColButton:SetHeight(18)
	theFrame.ResetColButton:SetHeight(24)
	--theFrame.ResetColButton:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 40, -216)
	theFrame.ResetColButton:SetPoint("TOP", theFrame, "BOTTOM", 0, -16)
...
function me:CreateClassColorSelection(parent) -- (4.2) ClassColor Config-Column
...
--	i=i+1
	...
	--theFrame.ResetColButton:SetHeight(18)
	theFrame.ResetColButton:SetHeight(24)
	--theFrame.ResetColButton:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 40, -212)
	theFrame.ResetColButton:SetPoint("TOP", theFrame, "BOTTOM", 0, -16)
...
------ LEVEL: Window.Tab.Column.Element

function me:CreateFilterRow(parent,label,header) -- (1.1.x) Row inside Config-Column
	local theFrame=CreateFrame("Frame",nil,parent)

	--theFrame:SetWidth(196)
	theFrame:SetWidth(WIDTH_SECTION) -- TODO smth like parent.getWidth() is required
...
function me:CreateSavedCheckbox(Text, parent, VarTop, VarName) -- (1.2.x) Row inside Config-Column
...
function me:UpdateFonts() -- (3.2.1) Update Font List
...
function me:RefreshFonts() -- (3.2.2) Refresh Font List
	local Fonts=SM:List("font")
...
function me:CreateSelectFont(parent) -- What's that?
	local frame=CreateFrame("Frame",nil,parent)
	frame:SetHeight(13)
	frame:SetWidth(180)
	frame.Text=frame:CreateFontString(nil,"OVERLAY","GameFontNormal")
