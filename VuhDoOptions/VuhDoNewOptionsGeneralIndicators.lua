...
--				["model"] = "VUHDO_INDICATOR_CONFIG.CUSTOM.MANA_BAR.HEIGHT",
--				["tooltip"] = VUHDO_I18N_TT_081,
--			},
			{
				["name"] = VUHDO_I18N_BAR_TEXTURE,
				["type"] = VUHDO_ENUMERATOR_CONSTRAINTS,
...
--		VUHDO_Msg(anIndex .. ": " .. aParent:GetName());
...
local function VUHDO_buildCustomComponents(aPanel, someCustomElements)
	...
	for tIndex, tElement in ipairs(someCustomElements) do
		...
		if (tComponent ~= nil) then
			...
			tComponent:SetPoint("TOPLEFT", aPanel:GetName(), "TOPLEFT", 10, tYCompOfs);
			tYCompOfs = tYCompOfs - tComponent:GetHeight();
		end
	end
