...
function VehicleBarMod:OnEnable()
	...
	if not self.bar then
		...
		self.bar:RegisterEvent("PLAYER_GAINS_VEHICLE_DATA")
...
function VehicleBar:ApplyConfig(config)
	...
	if not self.config.position.x then
		-- Default was: self:ClearSetPoint("CENTER", 120, 27)
		self:ClearSetPoint("TOP", 213, 0)
		self:SavePosition()
	end
...
function VehicleBar:OnEvent(event, arg1)
	...
	elseif event == "PLAYER_GAINS_VEHICLE_DATA" then
		self:UpdateButtonVisibility()
		DEFAULT_CHAT_FRAME:AddMessage("PLAYER_GAINS_VEHICLE_DATA")
	end
end
