...
function StickyFrames:StartMoving(frame, frameList, left, top, right, bottom)
	...
	-- Set OnUpdate event to the generated function
...
function StickyFrames:GetUpdateFunc(frame, frameList, offsetX, offsetY, left, top, right, bottom)
	return function()
		local x, y = GetCursorPosition()
		...
		frame:ClearAllPoints()
		frame:SetPoint("CENTER", UIParent, "BOTTOMLEFT", x + offsetX, y + offsetY)
...
function StickyFrames:GetCursorAnchor(frame)
	-- Calculate necessary variables
	local x, y = GetCursorPosition()
	local centerX, centerY, width, height = frame:GetCenter(), frame:GetSize()
	local divLX, divRX, divTY, divBY
	divLX, divRX = centerX - width / 6, centerX + width / 6
	divTY, divBY = centerY - height / 6, centerY + height / 6
	-- Determine which anchor point is closer to the cursor
	if (x < divLX) then
		if (y < divTY) then
			return "TOPLEFT" -- 3
		elseif (y > divBY) then
...
function StickyFrames:SnapFrame(frameA, frameB, left, top, right, bottom)
	...
	local lftA, topA, rgtA, btmA = frameA:GetLeft(), frameA:GetTop(), frameA:GetRight(), frameA:GetBottom()
	local lftB, topB, rgtB, btmB = frameB:GetLeft(), frameB:GetTop(), frameB:GetRight(), frameB:GetBottom()
	local snap = nil

	-- Translate into A's scale
	lftB, topB, rgtB, btmB = (lftB * sB) / sA, (topB * sB) / sA, (rgtB * sB) / sA, (btmB * sB) / sA

	if (btmA <= topB and btmB <= topA) then -- Frame A is above B
		...
		-- Interior Left
		if lftA <= (lftB + StickyFrames.rangeX) and lftA >= (lftB - StickyFrames.rangeX) then
			newX = lftB + wA
		...
		-- Interior Right
		if rgtA <= (rgtB + StickyFrames.rangeX) and rgtA >= (rgtB - StickyFrames.rangeX) then
			newX = rgtB - wA
		...
		if lftA <= (rgtB + StickyFrames.rangeX) and lftA >= (rgtB - StickyFrames.rangeX) then
			newX = rgtB + (wA - left)
			if frameB == UIParent or frameB == WorldFrame then newX = newX + left / 2 end
			snap = true
		end
		...
		if rgtA <= (lftB + StickyFrames.rangeX) and rgtA >= (lftB - StickyFrames.rangeX) then
			newX = lftB - (wA - right)
			if frameB == UIParent or frameB == WorldFrame then newX = newX - right / 2 end
			snap = true
		end
	...
	if (lftA <= rgtB and lftB <= rgtA) then
		...
		-- Interior Top
		if topA <= (topB + StickyFrames.rangeY) and topA >= (topB - StickyFrames.rangeY) then
			newY = topB - hA
		...
		-- Interior Bottom
		if btmA <= (btmB + StickyFrames.rangeY) and btmA >= (btmB - StickyFrames.rangeY) then
			newY = btmB + hA
		...
		if topA <= (btmB + StickyFrames.rangeY + bottom) and topA >= (btmB - StickyFrames.rangeY + bottom) then
			newY = btmB - (hA - top)
			if frameB == UIParent or frameB == WorldFrame then newY = newY - top / 2 end
			snap = true
		end
		...
		if btmA <= (topB + StickyFrames.rangeY - top) and btmA >= (topB - StickyFrames.rangeY - top) then
			newY = topB + (hA - bottom)
			if frameB == UIParent or frameB == WorldFrame then newY = newY + bottom / 2 end
			snap = true
		end
