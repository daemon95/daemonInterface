...
function Bartender4:UpdateModuleConfigs()
	-- Suppose addon is initially locked
	local unlock = false
	-- If it is nevertheless unlocked then temporarily lock it
	if not self.Locked then
		self:Lock()
		unlock = true
	end

	-- Sequentially ApplyConfig() for every registered addon module
	...
	-- Unlock temporarily locked addon
	if unlock then
		self:Unlock()
	end
end
