...
function PetBarMod:OnEnable()
	DEFAULT_CHAT_FRAME:AddMessage("== Pet Bar has been enabled!")
...
function PetBar:ApplyConfig(config)
	...
	if not self.config.position.x then
		-- Set PetBar position
		-- Default was: self:ClearSetPoint("CENTER", 0, 70)
		self:ClearSetPoint("BOTTOM", 0, 10 * 38)
		-- It's somewhere to add orientation options
--		self:SavePosition()
	end
